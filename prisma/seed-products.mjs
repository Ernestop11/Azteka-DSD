...
function parseImageUrl(imageStr) {
...
export async function seedProducts() {
  ...
  for (const record of records) {
    ...
    if (!sku && productId) {
      sku = `PROD-${String(productId).padStart(6, '0')}`.toUpperCase();
    }
    ...
    try {
      ...
      // Only add categoryId and brandId if they exist
      if (categoryId) {
      ...
      // Use upsert to handle existing products (SKU is unique)
